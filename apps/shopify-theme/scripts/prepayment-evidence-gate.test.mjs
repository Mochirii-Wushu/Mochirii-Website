...
test("prepayment evidence pins the canonical Website repository identity", () => {
  assert.equal(PREPAYMENT_GATE_POLICY.repository, "Mochirii-Wushu/Mochirii-Website");
});

test("prepayment evidence grants no provider or launch authority", () => {
  const manifest = JSON.parse(readFileSync(path.join(repositoryRoot, "apps/shopify-theme/ACTIVE-SOURCE-MANIFEST.v1.json"), "utf8"));
  assert.deepEqual(
    Object.fromEntries(
      ["providerMutationAuthorized", "paymentSetupAuthorized", "publicationAuthorized", "commerceAuthorized"]
        .map((key) => [key, manifest.authority[key]]),
    ),
    {
      providerMutationAuthorized: false,
      paymentSetupAuthorized: false,
