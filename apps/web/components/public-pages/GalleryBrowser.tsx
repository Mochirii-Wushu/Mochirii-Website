...
function localGalleryAssetPath(value: unknown, fallback = "") {
...
function normalizeGalleryItem(
  ...
  const full = approvedSubmissionId
    ? ""
    : localGalleryAssetPath(item.full || item.src);
  const thumb = approvedSubmissionId
    ? text(item.thumb)
    : localGalleryAssetPath(item.thumb || item.src || item.full);
...
export function GalleryBrowser({
  ...
  return (
    <>
      ...
      {totalVisible === 0 && (approvedFeedState === "ready" || approvedFeedState === "empty") ? (
        <p id="galleryEmpty" className="gallery-empty" role="status">
          {activeQuery ? "No images match this search." : "No images in this category yet."}
        </p>
      ) : null}
      {totalVisible === 0 &&
        (approvedFeedState === "ready" || approvedFeedState === "empty") &&
