...
test("OAuth consent return preserves only one bounded authorization identifier", () => {
  assert.equal(
  ...
  assert.equal(
    resolveAuthReturnPath("/oauth/consent?authorization_id=abc&extra=value"),
    "/account",
  );
  assert.equal(
    resolveAuthReturnPath("/oauth/consent?authorization_id=abc&authorization_id=def"),
    "/account",
  );
});
