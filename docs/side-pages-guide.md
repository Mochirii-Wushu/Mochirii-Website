...
- Retired `/raffle/rules` and version paths permanently redirect to `/raffle`;
  they do not own content, metadata, bundles, or sitemap entries.
...
- A versioned rules anchor renders only reviewed local `rules.versions[]` content; retired rules paths redirect to `/raffle`.
