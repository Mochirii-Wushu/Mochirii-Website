...
The Website repository owns the reviewed contract. A future isolated fixed-egress runtime would own electronic reward-provider traffic only after a separately approved architecture and release. Production must never depend on a developer workstation.
