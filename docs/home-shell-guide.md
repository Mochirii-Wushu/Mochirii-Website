...
The exact public-name, URL, and surface decision is recorded in
`docs/integrations/official-guild-profile-assets.md`; require
`npm run gate:official-guild-profile-public-names` to pass before publication.
Mark gates are separate and remain intentionally red while these links are text-only.
