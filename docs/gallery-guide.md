...
- The private-original contract accepts static JPEG, PNG, or WebP files up to 8 MiB, 4096 pixels on either edge, and 12.6 megapixels. Client checks provide prompt feedback, while service-side structural validation bound to the exact Storage object is the trusted moderation gate. The moderator must validate one selected item before a private preview is signed; the queue must never bulk-download unvalidated originals.
