...
| Unscoped function deployment or prune | Shared function deletion/overwrite | Exact six-name Reaper allowlist, individual deploys, no `--prune`, before/after 33-name/version/JWT readback, and merge freeze. |
...
| Automatic same-email identity linking, account collision, or pre-account takeover | An attacker links to or captures another member's account | Never auto-link identities by email alone; bind the verified provider subject through an authenticated, recent-session linking flow; require reauthentication for link and unlink; make recovery explicit; revoke affected grants and invalidate sessions after unlink, credential compromise, or identity reassignment; test collision and recovery paths. |
