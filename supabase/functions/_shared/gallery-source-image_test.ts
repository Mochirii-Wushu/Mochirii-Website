...
Deno.test("rejects WebP duplicate dimensions, mismatches, invalid order, and truncation", async () => {
  ...
  const zeroWidth = STATIC_VP8.slice();
  zeroWidth[26] = 0;
  zeroWidth[27] &= 0xc0;
  await expectError(zeroWidth, "source_image_webp_invalid");

  const zeroHeight = STATIC_VP8.slice();
  zeroHeight[28] = 0;
  zeroHeight[29] &= 0xc0;
  await expectError(zeroHeight, "source_image_webp_invalid");
});
...
Deno.test("enforces encoded-byte, edge, and pixel ceilings independently", async () => {
  ...
  await expectError(
    structuralVp8l(721, 1),
    "source_image_webp_dimensions_unsupported",
  );
  assert(
    (await validateGallerySourceImage(structuralVp8l(720, 720))).ok,
    "the proven libwebp source edge should remain accepted structurally",
  );
});
