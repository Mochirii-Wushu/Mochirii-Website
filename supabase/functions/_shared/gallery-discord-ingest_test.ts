...
Deno.test("ingest secrets use a fixed-length digest comparison", async () => {
  assert(
    await constantTimeSecretEquals("same-secret", "same-secret"),
    "equal secret rejected",
  );
  assert(
    !await constantTimeSecretEquals("same-secret", "other-secret"),
    "different secret accepted",
  );
  assert(
    !await constantTimeSecretEquals("short", "longer"),
...
Deno.test("request JSON is content-type checked and bounded while streaming", async () => {
  ...
  assert(
    (await readBoundedJsonRecord(valid, 64))?.ok === true,
    "valid JSON rejected",
  );
  ...
  assert(
    await readBoundedJsonRecord(oversized, 32) === null,
    "oversized JSON accepted",
  );
});
...
Deno.test("attachment redirects remain manual and inside the allowlist", async () => {
    ...
    fetcher: (async (input, init) => {
      ...
      return calls.length === 1
        ? new Response(null, {
          status: 302,
          headers: {
            Location: "https://cdn.discordapp.com/attachments/1/2/final.png",
          },
        })
  ...
  assert(
    calls.length === 2 && calls.every((call) => call.redirect === "manual"),
    "redirects were followed implicitly",
  );
...
Deno.test("attachment bodies and deadlines fail closed", async () => {
  ...
  try {
    ...
    oversized = error instanceof GalleryDiscordIngestError &&
  ...
  try {
    ...
    timedOut = error instanceof GalleryDiscordIngestError &&
