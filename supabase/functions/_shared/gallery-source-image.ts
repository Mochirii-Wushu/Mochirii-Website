...
// The immutable libwebp 1.6.0 full-decode boundary currently proves source
// WebP pixels only through 720px. JPEG and PNG retain the general source
// geometry limits above.
export const GALLERY_SOURCE_WEBP_MAX_EDGE = 720;
...
export async function validateGallerySourceBytes(
  ...
  if (
    mimeType === "image/webp" &&
    (dimensions.width > GALLERY_SOURCE_WEBP_MAX_EDGE ||
      dimensions.height > GALLERY_SOURCE_WEBP_MAX_EDGE)
  ) {
    return { ok: false, error: "source_image_webp_dimensions_unsupported" };
  }
  if (
