import {
  ...
  GALLERY_SOURCE_IMAGE_DECODE_VERSION,
...
Deno.test("returns private binary source transport with bounded metadata", async () => {
  ...
  assert(
    response.headers.get("X-Gallery-Submission-Id") ===
      "123e4567-e89b-42d3-a456-426614174000",
    "submission header",
  );
  assert(
    response.headers.get("X-Gallery-Source-Width") === "640",
    "width header",
  );
  assert(
    response.headers.get("X-Gallery-Source-Height") === "480",
    "height header",
  );
  assert(
    response.headers.get("X-Gallery-Source-Validated-At") ===
      validatedAt,
    "validation time header",
  );
  assert(
    response.headers.get("X-Gallery-Source-Decode-Version") ===
      GALLERY_SOURCE_IMAGE_DECODE_VERSION,
