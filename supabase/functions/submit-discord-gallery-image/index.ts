...
async function handleRequest(req: Request): Promise<Response> {
  ...
  if (insertError || !insertedData) {
    ...
    console.error("submit-discord-gallery-image submission insert failed", {
      ...
      category: insertError
        ? "database_insert_rejected"
        : "missing_inserted_row",
    });
    ...
    console.error(
      "submit-discord-gallery-image source validation commit failed",
      {
        ...
        commitCode: validationCommitError && "code" in validationCommitError
          ...
          : null,
        category: validationCommit.committed === true
          ? "validation_dependency_failed"
          : "validation_commit_rejected",
      },
    );
    return jsonResponse(
      {
