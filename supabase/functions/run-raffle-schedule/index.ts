...
async function promoteAlternates(
  ...
  for (const cycleValue of cycles || []) {
    ...
    const cycleId = safeString(cycle.id, 80);
    if (!cycleId) throw new Error("raffle_cycle_not_found");
    const { data, error: transitionError } = await adminClient.rpc(
      "advance_raffle_claim_schedule",
      {
        p_cycle_id: cycleId,
    ...
    const changedCount = Number(result.changedCount);
    if (!Number.isSafeInteger(changedCount) || changedCount < 0) {
      throw new Error("raffle_schedule_transition_invalid");
    }
