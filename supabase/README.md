...
The strict HMAC-only `submit-discord-gallery-image` replacement in this source tree is intentionally not compatible with that legacy in-repository caller and is not deployed by this change. The terminal replacement signer belongs to `Mochirii-Wushu/Reaper-Discord-Bot`; activation requires the separately approved fail-closed cutover packet in [`../docs/operations/discord-gallery-ingest-hmac-activation.md`](../docs/operations/discord-gallery-ingest-hmac-activation.md). Website retains verifier, nonce, reservation, RLS, migration, and moderation ownership only.
...
The repository's incumbent Interactions source validates Discord request signatures, answers PING, applies the configured guild/channel/role policy, defers the interaction, and calls the legacy ingest receiver in a background task. It remains deployment history, not a signer for the strict source candidate. The replacement `submit-discord-gallery-image` keeps `verify_jwt = false` because it authenticates exact request bytes with the versioned Reaper-owned HMAC contract. It then atomically consumes a nonce, validates the signed authorization context and exact payload, rechecks current linked-member eligibility, performs bounded transport and full image decode, and uses a private resumable reservation before finalizing a pending row. No legacy shared-secret fallback is present.
...
The source-only replacement makes Discord uploads resumable and idempotent by message/attachment ID plus the complete signed metadata and validated source digest. Every lease generation receives a fresh private object path derived from its lease token, so a late predecessor upload cannot overwrite the successor generation; expired private generation objects remain subject to a future reviewed retention policy. The producer must reject a missing, null, noncanonical, or filename-mismatched Discord `content_type` before signing. Website independently requires canonical JPEG, PNG, or static WebP MIME, exact URL channel/attachment identity, a safe-integer declared size no greater than 8 MiB, structural validation, full decode, and response/declared/decoded MIME agreement. JPEG/PNG retain the 4,096-edge and 12.6-million-pixel limits; the pinned libwebp 1.6.0 validator currently limits WebP to 720 pixels per edge. These original encoded bytes are not metadata-sanitized and must not be publicly released without the separate sanitized-derivative decision.

The terminal replacement signer source belongs to `Mochirii-Wushu/Reaper-Discord-Bot`; the Website repository must not duplicate it. The current hosted caller and any production cutover state require provider readback and are not inferred from this source candidate. The gallery slash command keeps `image` required; `title`, `subtitle`, and the Discord boolean opt-in stay optional:
