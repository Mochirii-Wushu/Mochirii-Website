...
-- This pipeline was not released before this migration. Refuse to bless any
-- pre-existing derivative that lacks cryptographic source provenance.
do $$
begin
  if exists (select 1 from private.gallery_social_derivatives) then
    raise exception
      'Unbound gallery social derivatives require manual quarantine before migration.';
  end if;
end;
$$;

alter table private.gallery_social_derivatives
  add column source_storage_object_id uuid not null,
  add column source_storage_object_version text,
  add column source_storage_object_updated_at timestamptz not null,
  add column source_sha256 text not null,
  add column derivation_method text not null,
  add constraint gallery_social_derivatives_source_sha256_check
    check (source_sha256 ~ '^[0-9a-f]{64}$'),
  add constraint gallery_social_derivatives_derivation_method_check
    check (derivation_method = 'jpeg-metadata-strip-v1');

-- New social consent is accepted only for a JPEG source. Historical rows are
-- left untouched and will remain ineligible at the queue boundary.
alter table public.gallery_submissions
  add constraint gallery_submissions_social_opt_in_jpeg_check
  check (
    (
      instagram_opt_in is not true
      and facebook_page_opt_in is not true
    )
    or mime_type = 'image/jpeg'
  ) not valid;

drop function if exists public.gallery_commit_moderation_with_social_derivative(
