...
create table public.raffle_cycles (
  ...
  constraint raffle_cycles_rules_check check (
    ...
    and rules_version_url = '/raffle/rules/' || rules_version
  ...
  constraint raffle_cycles_counts_check check (
    ...
    and (entrant_count is null or total_entry_count is null or total_entry_count between entrant_count * 5 and entrant_count * 10)
  )
);
...
create table public.raffle_entries (
  ...
  constraint raffle_entries_base_count_check check (base_entry_count in (0, 5)),
  constraint raffle_entries_frozen_count_check check (frozen_entry_count is null or frozen_entry_count between 5 and 10),
...
create table public.raffle_bonus_awards (
  ...
  constraint raffle_bonus_awards_bonus_key_check check (bonus_key in ('scheduled_activity', 'monthly_gathering', 'party_help', 'tip_resource', 'member_kudos')),
...
create table public.raffle_draws (
  ...
  constraint raffle_draws_counts_check check (
    entrant_count >= 3
    and total_entry_count between entrant_count * 5 and entrant_count * 10
  ),
...
begin
  ...
  set administrator_clearance_status = clearance_status,
      ...
      base_entry_count = case when eligibility_state = 'eligible' then 5 else 0 end,
...
begin
  ...
  if p_actor_id is null
    or p_revoke is null
    or p_bonus_key not in ('scheduled_activity', 'monthly_gathering', 'party_help', 'tip_resource', 'member_kudos')
...
begin
  ...
  insert into public.raffle_entries (
    ...
    p_now, null, case when resulting_status = 'eligible' then 5 else 0 end,
...
begin
  perform private.assert_raffle_service_caller();
  if p_ledger_hash !~ '^[0-9a-f]{64}$' or p_seed_hex !~ '^[0-9a-f]{64}$' or p_seed_hash !~ '^[0-9a-f]{64}$' then
    raise exception 'invalid_draw_evidence';
  end if;
  if p_algorithm_version <> 'mochirii-weighted-without-replacement-v1' then
    raise exception 'invalid_draw_algorithm_version';
  end if;
  if encode(extensions.digest(p_seed_hex, 'sha256'), 'hex') <> p_seed_hash then
    raise exception 'raffle_seed_hash_mismatch';
  end if;
  if jsonb_typeof(p_results) <> 'array' then raise exception 'invalid_draw_results'; end if;
  ...
  if draw_row.status = 'drawn' then
    ...
    if draw_row.ledger_hash <> p_ledger_hash
      or draw_row.seed_hex <> p_seed_hex
      or draw_row.seed_hash <> p_seed_hash
      or draw_row.algorithm_version <> p_algorithm_version
      or canonical_results <> p_results
      or stored_results <> canonical_results then
  ...
  if draw_row.status <> 'frozen'
    or draw_row.ledger_hash is null
    or draw_row.ledger_hash <> p_ledger_hash
    or draw_row.seed_hex is null or draw_row.seed_hex <> p_seed_hex
    or draw_row.seed_hash is null or draw_row.seed_hash <> p_seed_hash
    or draw_row.algorithm_version <> p_algorithm_version then
    raise exception 'raffle_ledger_hash_not_committed';
  end if;

  if canonical_results <> p_results then
