...
const REGISTRY_PATH = "docs/integrations/cross-repository-contract-registry.v1.json";
...
const REPOSITORIES = new Set([
  ...
  "Mochirii-Wushu/Reaper-Discord-Bot",
...
assertExactKeys(
  registry,
  ["$schema", "schemaVersion", "updatedDate", "registryStatus", "repositories", "contracts"],
  "registry",
);
...
if (
  ...
  fail("The v1 schema must reject unknown top-level, contract, artifact, and test fields.");
}
assertExactStringSet(
  schema.required,
  new Set(["$schema", "schemaVersion", "updatedDate", "registryStatus", "repositories", "contracts"]),
  "schema.required",
);
...
console.log(`Cross-repository contract registry OK (${registry.contracts.length} unversioned target contracts; no provider state asserted).`);
