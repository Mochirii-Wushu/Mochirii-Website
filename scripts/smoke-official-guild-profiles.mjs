import { isProfileProviderHost } from "./lib/profile-provider-boundary.mjs";
...
const headerProfiles = [
  {
    id: "facebook-page",
    name: "Facebook mochiriiguild external profile",
  ...
  {
    id: "youtube",
    name: "YouTube @MochiriiGuild external profile",
...
console.log("- Profile-provider and unexpected external requests before activation: 0");
...
function installDiagnostics(page, navigationUrl) {
  ...
  page.on("request", (request) => {
    ...
    if (isProfileProviderHost(url.hostname)) diagnostics.providerRequests.push(`${request.method()} ${safeUrl(request.url())}`);
    if (url.origin !== appOrigin && !permittedPreActivationOrigins.has(url.origin)) {
      diagnostics.unexpectedExternalRequests.push(`${request.method()} ${safeUrl(request.url())}`);
    }
...
function assertDiagnosticsClean(diagnostics, browserName, viewport) {
  ...
  const findings = [
    ...
    ...diagnostics.unexpectedExternalRequests.map((message) => `unexpected external request before activation: ${message}`),
  ];
