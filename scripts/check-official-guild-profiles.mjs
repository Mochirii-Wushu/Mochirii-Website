...
const blockedPublicNameDecision = '- Public-name decision: `BLOCKED_APPROVAL`';
...
for (const decision of [blockedPublicNameDecision, blockedMetaMarkDecision, blockedTikTokMarkDecision]) {
...
if (/^- (?:Public-name|Meta mark|TikTok mark) decision: `(?!BLOCKED_APPROVAL|BLOCKED_EXTERNAL)[^`]+`$/gmu.test(provenance)) {
...
assertIds("footer", FOOTER_GUILD_PROFILES, ["facebook-page", "instagram", "tiktok"]);
...
for (const hostname of [
  "facebook.com.example", "notfacebook.com", "tiktok.com.example", "mochirii.com",
]) {
...
if (requirePublicNameApproval && provenance.includes(blockedPublicNameDecision)) {
  failures.push("exact public-name approval is required before the profile shell may be published");
}
if (requireMetaMarks) {
...
if (requireAllMarks) {
  if (provenance.includes(blockedMetaMarkDecision) || provenance.includes(blockedTikTokMarkDecision)) {
    failures.push("approved Meta and TikTok mark decisions are required before all provider artwork may be published");
  }
