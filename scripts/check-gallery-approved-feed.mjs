...
const laterReleaseLayerSentinels = [
  "apps/web/components/ResponsiveGalleryMedia.tsx",
  "apps/web/lib/gallery/browser-state.ts",
  "apps/web/lib/gallery/browser-state_test.ts",
  "apps/web/lib/gallery/moderation-preview-client.ts",
  "apps/web/lib/gallery/moderation-preview-route.ts",
  "apps/web/lib/gallery/moderation-preview-server.ts",
  "apps/web/lib/gallery/moderation-preview-server-core.ts",
  "apps/web/lib/gallery/approved-feed_test.ts",
  "apps/web/lib/gallery/safe-preview.ts",
  "apps/web/lib/gallery/safe-preview_test.ts",
  "docs/integrations/gallery-public-media-delivery.md",
];
const laterReleaseLayerFiles = laterReleaseLayerSentinels.filter((path) =>
  existsSync(new URL(`../${path}`, import.meta.url))
);
const laterReleaseLayerPresent =
  laterReleaseLayerFiles.length === laterReleaseLayerSentinels.length;
if (
  laterReleaseLayerFiles.length > 0 &&
  !laterReleaseLayerPresent
) {
  failures.push(
    `Later Gallery release layer is incomplete (${laterReleaseLayerFiles.length}/${laterReleaseLayerSentinels.length} sentinel files).`,
  );
}
...
function readLaterReleaseLayer(path) {
  if (!laterReleaseLayerPresent) {
    optionalFilesSkipped.push(path);
    return null;
  }
  return read(path);
}
...
const galleryRouteComponent = readLaterReleaseLayer("apps/web/components/public-pages/route-pages/GalleryPage.tsx");
const galleryBrowser = readLaterReleaseLayer("apps/web/components/public-pages/GalleryBrowser.tsx");
const responsiveGalleryMedia = readLaterReleaseLayer("apps/web/components/ResponsiveGalleryMedia.tsx");
const galleryBrowserState = readLaterReleaseLayer("apps/web/lib/gallery/browser-state.ts");
const galleryBrowserStateTests = readLaterReleaseLayer("apps/web/lib/gallery/browser-state_test.ts");
const leaderDashboard = readLaterReleaseLayer("apps/web/components/member-workflow/LeaderDashboard.tsx");
const leaderDashboardParts = readLaterReleaseLayer("apps/web/components/member-workflow/LeaderDashboardParts.tsx");
const browserThumbnail = readLaterReleaseLayer("apps/web/lib/gallery-thumbnail.ts");
const moderationPreviewClient = readLaterReleaseLayer("apps/web/lib/gallery/moderation-preview-client.ts");
const moderationPreviewRoute = readLaterReleaseLayer("apps/web/lib/gallery/moderation-preview-route.ts");
const moderationPreviewServer = readLaterReleaseLayer("apps/web/lib/gallery/moderation-preview-server.ts");
const moderationPreviewServerCore = readLaterReleaseLayer("apps/web/lib/gallery/moderation-preview-server-core.ts");
...
const galleryModerationTests = read("supabase/functions/_shared/gallery-moderation_test.ts");
const galleryResponseSafety = read("supabase/functions/_shared/gallery-response-safety.ts");
const galleryResponseSafetyTests = read("supabase/functions/_shared/gallery-response-safety_test.ts");
const approvedFeedClient = readLaterReleaseLayer("apps/web/lib/gallery/approved-feed.ts");
const approvedFeedClientTests = readLaterReleaseLayer("apps/web/lib/gallery/approved-feed_test.ts");
const safePreview = readLaterReleaseLayer("apps/web/lib/gallery/safe-preview.ts");
const safePreviewTests = readLaterReleaseLayer("apps/web/lib/gallery/safe-preview_test.ts");
...
const rejectedCleanup = read("supabase/functions/delete-rejected-gallery-submission/index.ts");
const discordGallerySubmit = read("supabase/functions/submit-discord-gallery-image/index.ts");
...
const galleryGuide = readLaterReleaseLayer("docs/gallery-guide.md");
const moderationRunbook = readLaterReleaseLayer("docs/member-gallery-moderation-runbook.md");
const deliveryContract = readLaterReleaseLayer("docs/integrations/gallery-public-media-delivery.md");
...
const edgeFunctionListMatch = edgeTypeCheck.match(/const functions = \[([\s\S]*?)\];/);
const edgeTypeFunctions = edgeFunctionListMatch
  ? [...edgeFunctionListMatch[1].matchAll(/"([^"]+)"/g)].map((match) => match[1]).sort()
  : [];
const configuredFunctions = functionBlocks.map(([, name]) => name).sort();
assert(Boolean(edgeFunctionListMatch), "Supabase function inventory: the Edge type checker function list is missing.");
assert(
  JSON.stringify(configuredFunctions) === JSON.stringify(edgeTypeFunctions),
  "Supabase function inventory: config.toml and the Edge type checker must name the same functions.",
);
assert(
  verifyJwtTrue.length + verifyJwtFalse.length === functionBlocks.length,
  "Supabase function inventory: every configured function must declare verify_jwt explicitly.",
);
...
for (const [functionName, expectedVerifyJwt] of [
  ["list-gallery-review-queue", true],
  ["moderate-gallery-submission", true],
  ["delete-rejected-gallery-submission", true],
  ["withdraw-gallery-publication-consent", true],
  ["list-approved-gallery-submissions", false],
  ["submit-discord-gallery-image", false],
]) {
  const block = functionBlocks.find(([, name]) => name === functionName);
  assert(Boolean(block), `Supabase function inventory: ${functionName} is not configured.`);
  assert(
    Boolean(block && new RegExp(`verify_jwt\\s*=\\s*${expectedVerifyJwt}`).test(block[2])),
    `Supabase function inventory: ${functionName} must retain verify_jwt=${expectedVerifyJwt}.`,
  );
}
...
if (approvedFeedClientTests !== null) {
  ...
  assertIncludes("full repository check", checkAll, '"test:gallery-approved-feed-client"');
}
if (galleryBrowserStateTests !== null) {
  assertIncludes("package scripts", packageJson, '"test:gallery-browser-state"');
  assertIncludes("full repository check", checkAll, '"test:gallery-browser-state"');
}
if (safePreviewTests !== null) {
  assertIncludes("package scripts", packageJson, '"test:gallery-safe-preview"');
...
[
  ...
  "Supabase Edge Function manifest inventory does not match the reviewed function list.",
  "--frozen=true",
  "Repository Deno lock audit",
].forEach((snippet) => assertIncludes("Edge dependency validation", edgeTypeCheck, snippet));
...
if (
  galleryBrowser !== null &&
  (galleryBrowser.includes("setRandomSeed") || galleryBrowser.includes("createRandomSeed"))
) {
...
const reviewQueueDto = reviewQueue.slice(
  reviewQueue.indexOf("queue.push({"),
  reviewQueue.indexOf("summary.shown"),
);
[
  "storageBucket",
  "storagePath",
  ...
  "sourceValidatedAt",
  "instagramOptInAt",
  "instagramOptInSource",
  "instagramOptInCopyVersion",
  "facebookPageOptInAt",
  "facebookPageOptInSource",
  "facebookPageOptInCopyVersion",
  "facebookPageOptInContractVersion",
].forEach((snippet) =>
  assertNotIncludes("private Gallery review DTO", reviewQueueDto, snippet)
);
[
  ...
  assertIncludes("bounded Gallery review DTO", reviewQueueDto, snippet)
);
assertIncludes(
  "bounded Gallery moderator profile projection",
  reviewQueue,
...
[
  ...
  '"discord_lookup_timeout"',
  '"discord_response_invalid"',
  '.select("member_status,discord_user_id,discord_member_pending")',
  '"discord_identity_drift"',
  '"member_not_active"',
].forEach((snippet) =>
  assertIncludes("bounded live Discord moderator lookup", galleryModerationShared, snippet)
);
assertNotIncludes(
  "bounded live Discord moderator lookup",
  galleryModerationShared,
  "retry-after",
);
[
  "moderator role configuration fails closed on drift",
  "live moderator lookup is bounded, redirect-safe, and single-attempt",
  "live moderator lookup times out and fails closed",
  "live moderator deadline covers a stalled response body",
  "live moderator lookup never exposes role ids or rate-limit headers",
  "live moderator lookup rejects oversized or malformed provider bodies",
].forEach((snippet) =>
  assertIncludes("live Discord moderator tests", galleryModerationTests, snippet)
);
...
[
  ...
  assertNotIncludes("private Gallery source response", sourceDecode, snippet)
);
...
[
  ...
  "instagram_publish_requires_separate_review",
  "safeGalleryModerationSubmission(updatedSubmission)",
].forEach((snippet) =>
  assertIncludes("separate Gallery publication confirmation", moderation, snippet)
);
assertNotIncludes(
  "safe Gallery moderation response",
  moderation,
...
[
  ...
  "publishInstagramJob(",
  "bodyResult.body.message",
  "bodyResult.body.caption",
  "bodyResult.body.alt_text",
].forEach((snippet) =>
  assertNotIncludes("Gallery approval provider boundary", moderation, snippet)
);
...
[
  ...
  assertNotIncludes("pre-HMAC Discord Gallery submission", discordGallerySubmit, snippet)
);
assertIncludes(
  "pre-HMAC Discord Gallery submission",
  discordGallerySubmit,
  'Deno.env.get("DISCORD_GALLERY_INGEST_SECRET")',
);
const rejectedCleanupResponse = rejectedCleanup.slice(
  rejectedCleanup.lastIndexOf("return jsonResponse({"),
);
assertNotIncludes(
  "rejected Gallery cleanup response",
  rejectedCleanupResponse,
  "storagePath",
);
assertNotIncludes(
  "rejected Gallery cleanup response",
...
[
  [galleryModerationShared, "userError?.message"],
  [galleryModerationShared, "identityError.message"],
  [reviewQueue, "result.error.message"],
  [reviewQueue, "submissionError.message"],
  [reviewQueue, "eventError.message"],
  [reviewQueue, "profileError.message"],
  [moderation, "lookupError.message"],
  [moderation, "commitError.message"],
  [rejectedCleanup, "lookupError.message"],
  [rejectedCleanup, "storageError.message"],
  [rejectedCleanup, "deleteError.message"],
].forEach(([source, snippet], index) =>
  assertNotIncludes(`Gallery raw error logging source ${index + 1}`, source, snippet)
);
...
if (optionalFilesSkipped.length) {
  console.log(`Deferred checks for files owned by a later release layer: ${optionalFilesSkipped.join(", ")}.`);
}
