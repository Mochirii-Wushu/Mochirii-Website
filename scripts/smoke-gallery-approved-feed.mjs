  ...
  try {
    ...
    const state = await image.evaluate((element) => {
      ...
      return {
        ...
        ariaBusy: element.closest(".responsive-gallery-media")?.hasAttribute("aria-busy") || false,
      };
    });
    assert(state.naturalWidth > 0, "JavaScript-disabled Gallery did not load its static thumbnail.");
    assert(!state.ariaBusy, "JavaScript-disabled Gallery thumbnail remained accessibility-busy without hydration.");
