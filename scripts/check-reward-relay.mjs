...
if (!failures.length) {
  ...
  const routeShellSource = readFileSync(join(root, "apps", "web", "components", "SiteRouteShell.tsx"), "utf8");
  requireAll(routeShellSource, [
    'pathname === "/raffle/claim"',
    'pathname === "/raffle/claim/"',
    "if (isIsolatedRewardClaimPath(pathname)) return children;",
  ], "claim route analytics and shared-shell isolation");

  const nextConfigSource = readFileSync(join(root, "apps", "web", "next.config.ts"), "utf8");
  requireAll(nextConfigSource, [
    'source: "/raffle/claim"',
    ...
    '"connect-src \'self\'"',
    'value: "Cookie"',
  ], "claim route private response and same-origin execution policy");
  requireNone(
    nextConfigSource.slice(
      nextConfigSource.indexOf("const rewardClaimContentSecurityPolicy"),
      nextConfigSource.indexOf("const securityHeaders"),
    ),
    ["vercel-insights.com", "supabase.co", "discord.com", "spotify.com", "http:"],
    "claim route third-party content policy",
  );
