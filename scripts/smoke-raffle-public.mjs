...
await verifyRetiredRuleRedirects();
...
async function verifyRetiredRuleRedirects() {
  for (const path of ["/raffle/rules", "/raffle/rules/example-cycle"]) {
    try {
      ...
      if (response.status !== 308 || destination?.pathname !== "/raffle") {
        failures.push(`${path}: expected one permanent 308 redirect to /raffle, received HTTP ${response.status} and ${JSON.stringify(location)}.`);
      }
    } catch (error) {
      failures.push(`${path}: redirect verification failed: ${safeMessage(error)}`);
    }
  }
}
