...
requireIncludes(workflowPath, workflow, [
  ...
  "gh api repos/Mochirii-Wushu/Mochirii-Website/commits/main",
...
requireIncludes(recoveryWorkflowPath, recoveryWorkflow, [
  ...
  "recovery/recovery.tar.age.manifest",
  "recovery/recovery.tar.age.manifest.sig",
...
requireIncludes(updaterPath, updater, [
  ...
  'install_file_atomic "${sources[5]}" "${targets[5]}"',
...
requireIncludes(harnessPath, harness, [
  ...
  "Authenticated backup-producer evidence accepted modified ciphertext.",
  "Authenticated backup-producer evidence accepted a modified manifest.",
  "Authenticated backup-producer evidence accepted an untrusted producer key.",
  "Authenticated producer evidence accepted a different payload identity.",
  ...
  "The boot guard accepted an active private-media cutover.",
  "The boot guard accepted an active restore-recovery intent.",
